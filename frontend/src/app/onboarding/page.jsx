...
export default function OnboardingPage() {
  ...
  const finish = async () => {
    ...
    } catch { toast.error('Failed to save'); router.push('/dashboard'); }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-8 bg-slate-50">
      <div className="w-full max-w-lg">
        ...
        <div className="flex gap-1 mb-8">{[0,1,2,3,4].map(i => <div key={i} className={`h-1 flex-1 rounded-full ${i <= step ? 'bg-brand-500' : 'bg-slate-200'}`} />)}</div>
        ...
        {step === 1 && <div><h2 className="font-display font-bold text-2xl mb-4">Which exam?</h2>
        ...
        {step === 2 && <div><h2 className="font-display font-bold text-2xl mb-4">Target level?</h2>
