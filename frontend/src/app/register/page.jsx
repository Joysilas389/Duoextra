...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Account created! Let\'s set up your learning plan.');
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 mb-6">
            ...
            <span className="font-display font-bold text-2xl">DuoExtra</span>
          </Link>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                type="text" value={form.displayName} onChange={update('displayName')}
        ...
        <p className="text-center text-sm text-slate-500 mt-6">
          Already have an account? <Link href="/login" className="text-brand-600 font-semibold">Sign in</Link>
        </p>
      </div>
    </div>
  );
